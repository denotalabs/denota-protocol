...
import {SimpleCashTimelock} from "../src/modules/SimpleCashTimelock.sol";
import {RegistrarTest} from "./Registrar.t.sol";

contract SimpleCashTimelockTest is RegistrarTest {
    SimpleCashTimelock public SIMPLE_CASH_TIMELOCK;

    function setUp() public override {
        super.setUp();  // init registrar, tokens, and their labels   
        SIMPLE_CASH_TIMELOCK = new SimpleCashTimelock(address(REGISTRAR));
        vm.label(address(SIMPLE_CASH_TIMELOCK), "SimpleCashTimelock");
    }
    ...
    function _setupThenWrite(
        ...
        _registrarModuleWhitelistToggleHelper(SIMPLE_CASH_TIMELOCK, false); // startedAs=false
        ...
        uint256 notaId = _registrarWriteHelper(caller, address(DAI), escrowed, instant, owner, SIMPLE_CASH_TIMELOCK, initData);
